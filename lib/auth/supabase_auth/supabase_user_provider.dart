...
class ChurchFintechAdminSupabaseUser extends BaseAuthUser {
  ChurchFintechAdminSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> churchFintechAdminSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ChurchFintechAdminSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
